...
const ManageImages = () => {
    ...
    const handleSwitch = async (id, isDisplay) => {
        const newCategory = isDisplay
            ? "taken-by-guest"
            : "taken-by-guest-noDisplay";

        const  data  = await isDisplayToTrueOrFalse(id, isDisplay);

        console.log("data",data);

        // Update the "with display" state
        setGuestImages((prev) =>
            prev.map((item) =>
                item.id === id
        ...
        // Update the "without display" state
        setGuestNoDisplayImages((prev) =>
            prev.map((item) =>
                item.id === id
                    ? { ...item, category: newCategory }
                        ...
                        >
                            {filteredImages.map((item, index) => (
                                <div key={item.id}  className="pt-5">
                                    <label className={`btn-showHide btn btn-xs ${ item.category === "taken-by-guest" ? "btn-primary-outline" : "btn-primary-outline" } active-state`} >
                                        <input
                                            className="hidden"
                                            type="checkbox"
                                            checked={item.category === "taken-by-guest"}
                                            onChange={(e) => handleSwitch(item.id, e.target.checked)}
                                        />
                                       {item.category === "taken-by-guest" ? (
                                        <>
                                            <svg
